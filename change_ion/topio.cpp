...
int readTOP(const char* filename, TOPData* topData){
	...
	if (topFile != NULL ){
		while(safe_fgets(buffer, BUF_SIZE, topFile) != NULL){
			...
			if(strstr(buffer, "[ native ]") != 0){
				printf("Counting natives...\n");
				topData->nativesCount = countRowsInTOP(topFile);
				printf("%d found.\n", topData->nativesCount);
			}
		...
		topData->natives = (TOPPair*)calloc(topData->nativesCount, sizeof(TOPPair));
	...
	while(safe_fgets(buffer, BUF_SIZE, topFile) != NULL){
		...
		if(strstr(buffer, "[ native ]") != 0){
			printf("Reading natives...\n");
			count = 0;
			while(count < topData->nativesCount){
				topData->natives[count] = readPairLineFromTOP(topFile);
				if(topData->natives[count].i != -1){
					count++;
				}
			}
		}
