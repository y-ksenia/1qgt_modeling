...
o=open("data.dna",'w')
N = input('Write number of DNA bp:')
#N = int(sys.argv[1])
l_0 = 3.4 #[A] --- the distance between bps
...
#ion parameters
r0 = 5
mM = input('Write NaCl concentration [mM]:')
#mM = float(sys.argv[3])
cation_charge = 1
anion_charge = -1
...
for ich in range(n_dna_chains):	#molecule 0 and 1
	...
	for c in range(2):		#chain 0 and 1
		...
		for bp in range(len(sequences[chain_number-1])):	
			...
			z = (11.065/2)*(bp*35*pi/180) - l_0 * N / 2 #recenter
			if bp == 0:
				charge = 0.0
			#bead_type += 1
...
print atom_number-2*n_dna_chains
