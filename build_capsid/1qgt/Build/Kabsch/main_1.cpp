...
int main(int argc, char *argv[]){
	...
	writePDB("1qgt_full_segname.vdb", &refpdb);
	PDB pdb;
	PDB rotpdb;
	char filename[1024] = "A.pdb";
	...
	for(segid = 1; segid <= 60; segid++){
		for(chain = 'A'; chain <= 'D'; chain++){
			...
			sprintf(filename, "Chains/%c_%d.pdb", chain, segid);
