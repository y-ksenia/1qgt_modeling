...
void readAtoms(FILE* psfFile, PSF* psfData){
	...
	} while (strstr(buffer, "!NATOM") == NULL && feof(psfFile) == 0);
	if(feof(psfFile) == 0){
		pch = strtok(buffer, " ");
		psfData->natom = atoi(pch);
	} else {
		printf("There is no atoms section in PSF file.\n");
		psfData->natom = 0;
	}
...
void readBonds(FILE* psfFile, PSF* psfData){
	...
	if(feof(psfFile) == 0){
		...
		printf("There is no bonds section in PSF file.\n");
		psfData->nbond = 0;
	}
...
void readAngles(FILE* psfFile, PSF* psfData){
	...
	if(feof(psfFile) == 0){
		...
		printf("There is no angles section in PSF file.\n");
		psfData->ntheta = 0;
	}
...
void readDihedrals(FILE* psfFile, PSF* psfData){
	...
	if(feof(psfFile) == 0){
		...
		printf("There is no dihedrals section in PSF file.\n");
		psfData->nphi = 0;
	}
...
void readImpropers(FILE* psfFile, PSF* psfData){
	...
	do{
		safe_fgets(buffer, BUF_SIZE, psfFile);
	} while (strstr(buffer, "!NIMPHI") == NULL && feof(psfFile) == 0);
	if(feof(psfFile) == 0){
		pch = strtok(buffer, " ");
		psfData->nimphi = atoi(pch);
	} else {
		printf("There is no improper dihedrals section in PSF file.\n");
		psfData->nimphi = 0;
	}
...
void readCMAP(FILE* psfFile, PSF* psfData){
	...
	do{
		safe_fgets(buffer, BUF_SIZE, psfFile);
	} while (strstr(buffer, "!NCRTERM") == NULL && feof(psfFile) == 0);
	if(feof(psfFile) == 0){
		pch = strtok(buffer, " ");
		psfData->ncmap = atoi(pch);
	} else {
		printf("There is no CMAP section in PSF file.\n");
		psfData->ncmap = 0;
	}
...
void readExclusions(FILE* psfFile, PSF* psfData){
	...
	if(feof(psfFile) == 0){
		...
		printf("There is no exclusions section in PSF file.\n");
		psfData->nnb = 0;
	}
